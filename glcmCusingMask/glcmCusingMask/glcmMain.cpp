...
int main(){
	...
	imgMask = cvLoadImage("zMask.bmp", 0);
	// image zMask has following matrix:
	// 0 0 255 255
	// 0 0 0   255
	// 0 0 0   255
	// 0 0 255 255
	...
	// prinitng the Mask values
	cout << endl << "Printing the imageMask vaules: " << endl << endl;
	for(int y = 0; y < imgMask->height; y++){
		for(int x = 0; x < imgMask->width; x++){
			intensity = imgMask->imageData[imgMask->widthStep * y + x * 1];
			cout << (unsigned int)intensity << "   ";
		}
		cout << endl;
	}
	...
	//just for varification the pGray has the same value as the image
	cout << endl << "Printing the Mask vaules from **pMask: " << endl << endl;
	for(int y = 0; y < imgMask->height; y++){
		for(int x = 0; x < imgMask->width; x++){
			intensity = pMask[y][x];
			cout << (unsigned int)intensity << "   ";
	...
	pMatrix = CoOcMat_Angle_135(distance, pGray, pMask, rows, cols, toneLUT, toneCount);
